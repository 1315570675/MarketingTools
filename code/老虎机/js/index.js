$(function () {
    ...
    function clickFn() {
        ...
        if(isWin){
			...
			var random1 = random[Math.floor(Math.random() * random.length)]+1;
			var random2 = random[Math.floor(Math.random() * random.length)]+1;
			var random3 = random[Math.floor(Math.random() * random.length)]+1;
        ...
        for (var i = 1; i <= random1; i++) {
            timer1 = setTimeout(animate(random1 - 40, i, 0), i*i);//第二个值越大，慢速旋转时间越长
        }
		for (var i = 1; i <= random2; i++) {
			timer2 = setTimeout(animate(random2 - 40, i, 1), 2 * i * i);
		}
		for (var i = 1; i <= random3; i++) {
			timer3 = setTimeout(animate(random3 - 40, i, 2), 3 * i * i);
		}
